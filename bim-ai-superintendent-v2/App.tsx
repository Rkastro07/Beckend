...
import { listFloors, analyzeFloorV2 } from './services/api';
...
export default function App() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const result = await analyzeFloorV2(ifcFile, plyFile, selectedFloorId, ifcToken);
      setAnalysisResult(result);
      setActiveTab(AppTab.VISTORIA);
    } catch (e) {
      console.error(e);
      alert("Falha ao processar análise v2.");
    } finally {
      ...
      <main className="flex-1 ml-80 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 px-8 h-16 flex items-center justify-between shrink-0 z-10">
          ...
          <div className="text-xs text-slate-400 flex items-center gap-3">
            <span className="px-2 py-1 bg-cyan-50 text-cyan-700 rounded font-semibold">
              Sonata + OBB (filtragem por bbox IFC)
            </span>
            <span>
              {analysisResult ? `Análise: ${new Date(analysisResult.timestamp!).toLocaleString()}` : 'Aguardando processamento'}
            </span>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-auto bg-slate-50">
          {activeTab === AppTab.VISTORIA && (
            // DataView é reaproveitado do legado; analysisMode="bbox" faz ele
            // tratar `result` como única fonte (sem branches de compare/AI/instances).
            <DataView result={analysisResult} resultAI={null} analysisMode="bbox" />
          )}
          {activeTab === AppTab.RELATORIO && (
            <ReportView result={analysisResult} csvContent={csvContent} />
          )}
          {activeTab === AppTab.VOICE && (
            <VoiceAssistant result={analysisResult} csvContent={csvContent} />
          )}
        </div>
      </main>
    </div>
