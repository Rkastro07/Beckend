...
export const DataView: React.FC<{
  ...
  return (
    <div className="flex flex-col w-full p-6 gap-6 bg-slate-50">
      ...
      {/* ── Seção: Adições (instâncias scan sem par IFC — pipeline v2) ── */}
      {Array.isArray(displayResult.adicoes) && displayResult.adicoes.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 mt-6">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <span className="px-2 py-1 bg-sky-100 text-sky-700 rounded text-xs">ADIÇÃO</span>
                Construído fora do plano
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Instâncias detectadas no scan que <strong>não têm correspondência</strong> no IFC.
                Total: {displayResult.adicoes.length}.
              </p>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs uppercase text-slate-500">
                <tr>
                  <th className="text-left px-4 py-2 font-medium">Classe (Sonata)</th>
                  <th className="text-right px-4 py-2 font-medium">N pts</th>
                  <th className="text-right px-4 py-2 font-medium">Conf.</th>
                  <th className="text-right px-4 py-2 font-medium">Volume (m³)</th>
                  <th className="text-left px-4 py-2 font-medium">Centroide (x,y,z)</th>
                </tr>
              </thead>
              <tbody>
                {displayResult.adicoes.map((a: any, idx: number) => (
                  <tr key={idx} className="border-b border-slate-100 bg-sky-50/40 hover:bg-sky-50">
                    <td className="px-4 py-2 font-medium text-sky-700">{a.scan_class}</td>
                    <td className="px-4 py-2 text-right text-slate-600">{a.n_pts?.toLocaleString?.() || a.n_pts}</td>
                    <td className="px-4 py-2 text-right text-slate-600">{a.mean_conf?.toFixed?.(2) ?? '-'}</td>
                    <td className="px-4 py-2 text-right text-slate-600">{a.volume?.toFixed?.(2) ?? '-'}</td>
                    <td className="px-4 py-2 text-slate-600 text-xs font-mono">
                      ({a.centroid?.[0]?.toFixed?.(2)}, {a.centroid?.[1]?.toFixed?.(2)}, {a.centroid?.[2]?.toFixed?.(2)})
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* ── Meta do pipeline Sonata+OBB ── */}
      {displayResult.meta && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 mt-6 p-6">
          <h3 className="text-sm font-bold text-slate-700 mb-3">Sonata + OBB — Telemetria</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            {typeof displayResult.meta?.tempo_sonata_s === 'number' && (
              <div className="bg-slate-50 rounded p-2 border border-slate-100">
                <div className="text-slate-500">Sonata inferência</div>
                <div className="font-mono text-slate-900">{displayResult.meta.tempo_sonata_s}s</div>
              </div>
            )}
            {typeof displayResult.meta?.subprocess_seconds === 'number' && (
              <div className="bg-slate-50 rounded p-2 border border-slate-100">
                <div className="text-slate-500">Subprocess total</div>
                <div className="font-mono text-slate-900">{displayResult.meta.subprocess_seconds.toFixed(1)}s</div>
              </div>
            )}
            {displayResult.meta?.pipeline && (
              <div className="bg-slate-50 rounded p-2 border border-slate-100">
                <div className="text-slate-500">Pipeline</div>
                <div className="font-mono text-slate-900">{displayResult.meta.pipeline}</div>
              </div>
            )}
            {displayResult.meta?.pavimento && (
              <div className="bg-slate-50 rounded p-2 border border-slate-100">
                <div className="text-slate-500">Pavimento</div>
                <div className="font-mono text-slate-900">{displayResult.meta.pavimento}</div>
              </div>
            )}
            {displayResult.meta?.verdicts_raw && Object.entries(displayResult.meta.verdicts_raw).map(([k, v]: any) => (
