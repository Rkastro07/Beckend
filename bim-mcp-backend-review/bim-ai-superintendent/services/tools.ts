...
export interface ScanParedesResult {
  ...
  classificacao?: [number, number, number, number, string][];
...
export const scanParedes = (sid: string, zlo: number, zhi: number,
                            ...
                            fatias?: ScanFatias): Promise<ScanParedesResult> =>
...
export const parsePlanta = async (file: File, escala?: number,
                                  espDefault = 0.15,
                                  pavimento?: string): Promise<ModeloPlanta> => {
  ...
  const r = await postTool('/api/planta/importar', file, params);
