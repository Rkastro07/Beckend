...
export const PlantaEditor: React.FC<PlantaEditorProps> = ({
  ...
  const carregarArquivo = async (file: File, pavimento?: string) => {
    setBusy(true); setErro(null); setResultado(null); setSel(null); setSelVertice(null);
    try {
      const m = await parsePlanta(file, undefined, 0.15, pavimento);
      ...
      <div className="p-8">
        ...
        <label className="mt-6 flex flex-col items-center justify-center gap-3 h-64 max-w-2xl
          ...
          <span className="text-sm text-slate-600 font-medium">
            {busy ? 'Importando geometria...' : 'Solte IFC, IFCZIP, DXF ou SVG'}
          </span>
          <span className="text-xs text-slate-400">
            IFC preserva objetos e pavimentos; DXF/SVG preservam vetores
          </span>
          <span className="text-[11px] text-slate-400">
            OBJ, USDZ, PLY e E57 entram pela aba Scan → BIM
          </span>
          <input type="file" accept=".ifc,.ifczip,.dxf,.svg" className="hidden"
...
const Cabecalho: React.FC<{ titulo?: string; subtitulo?: string }> = ({ titulo, subtitulo }) => (
  <div>
    ...
    <p className="text-sm text-slate-500">
      {subtitulo ?? 'Importe IFC, IFCZIP, DXF ou SVG; revise os objetos e gere um IFC editável.'}
    </p>
  </div>
);
...
const PainelSelecionado: React.FC<{
    ...
    return (
      <div className="bg-white rounded-xl border border-blue-200 p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-slate-800">{p.nome || 'Parede'}</span>
        ...
        {p.altura !== undefined && (
          <Campo label="Altura (m)">
            <input type="number" step={0.05} min={0.1} value={p.altura}
                   onChange={(e) => patchParede(p.id, { altura: +e.target.value })} className={numInput} />
          </Campo>
        )}
  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-slate-800">{a.nome || 'Esquadria'}</span>
        <button onClick={apagar} className="text-rose-500 hover:text-rose-700"><Trash2 className="w-4 h-4" /></button>
      </div>
      ...
      {a.altura !== undefined && (
        <Campo label="Altura (m)">
          <input type="number" step={0.05} min={0.1} value={a.altura}
                 onChange={(e) => patchAbertura(a.id, { altura: +e.target.value })} className={numInput} />
        </Campo>
      )}
      {a.peitoril !== undefined && a.tipo === 'window' && (
        <Campo label="Peitoril (m)">
          <input type="number" step={0.05} min={0} value={a.peitoril}
                 onChange={(e) => patchAbertura(a.id, { peitoril: +e.target.value })} className={numInput} />
        </Campo>
      )}
