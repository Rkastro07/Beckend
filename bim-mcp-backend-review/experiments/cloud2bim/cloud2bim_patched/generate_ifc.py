...
class IFCmodel:
    ...
    def create_slab(self, slab_name, points, slab_z_position, slab_height,
                    material_name, components=None):
        ...
        component_coordinates = components if components else [points]
        slab_extrusions = []
        for component in component_coordinates:
            clean_points = []
            for point in component:
                coordinates = (float(point[0]), float(point[1]))
                if not clean_points or coordinates != clean_points[-1]:
            ...
            polygon_points = [
                ...
                for coordinates in clean_points
            ]
            ...
            polyline_profile = self.ifc_file.create_entity(
                ...
                OuterCurve=self.ifc_file.create_entity(
                    "IfcPolyline", Points=polygon_points)
            )
            slab_extrusions.append(
                self.create_extruded_solid(
                    polyline_profile,
                    axis_placement_extrusion,
                    slab_extrusion_direction,
                    slab_height,
                )
            )
        if not slab_extrusions:
            raise ValueError("Slab requires at least one valid polygon component.")
        shape_rep = self.create_shape_representation(
            self.geom_rep_sub_context,
            "Body",
            "SweptSolid",
            slab_extrusions,
        )
    ...
    def create_wall(self, wall_placement, product_definition_shape,
                    description="Wall Description", name="Wall Name",
                    tag="Wall Tag", object_type="Wall"):
    ...
    def create_property_single_value(self, property_name: str, value):
        if isinstance(value, bool):
            nominal_value = self.ifc_file.create_entity("IfcBoolean", value)
        elif isinstance(value, int):
            nominal_value = self.ifc_file.create_entity("IfcInteger", value)
        elif isinstance(value, float):
            nominal_value = self.ifc_file.create_entity("IfcReal", value)
        else:
            nominal_value = self.ifc_file.create_entity(
                "IfcText", str(value))
