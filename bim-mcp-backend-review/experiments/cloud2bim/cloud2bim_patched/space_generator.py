...
def process_centerlines(walls_local, distance_threshold_extension, min_wall_length, plot=True):
    ...
    # Zone closure may extend/split axes, but those edits belong only to the
    # space-boundary calculation. Mutating the caller's wall dictionaries here
    # changed preview-approved geometry before the IFC writer saw it.
    walls_working = copy.deepcopy(walls_local)
    not_connected_walls = find_disconnected_walls(walls_working)
    if plot:
        plot_wall_center_lines(not_connected_walls, 'Walls without start-end point in intersection')
    extended_walls = extend_to_centerline(not_connected_walls, walls_working, distance_threshold_extension)
    if plot:
        plot_wall_center_lines(extended_walls, 'Walls extended to centerline')
    processed_walls = process_disconnected_walls(copy.deepcopy(walls_working), extended_walls, min_wall_length)
    if plot:
...
def spaces_from_closed_axis_polygons(space_dimensions, walls_local,
                                     minimum_area=0.5):
    """Build room volumes only from polygons closed by real wall axes.

    ``extract_space_dimensions`` already uses Shapely ``polygonize`` and
    therefore returns a polygon only when the wall graph is closed.  Insets
    the center-line polygon by half of the median wall thickness so the Space
    does not overlap the wall bodies.  No missing edge is synthesized.
    """
    if not walls_local:
        return {}
    thicknesses = [
        float(wall['thickness']) for wall in walls_local
        if np.isfinite(wall.get('thickness', np.nan))
        and float(wall['thickness']) > 0.0
    ]
    inset = 0.5 * float(np.median(thicknesses)) if thicknesses else 0.0
    zone_dict = {}
    for space_name, coordinates in space_dimensions.items():
        polygon = Polygon(coordinates)
        if not polygon.is_valid:
            polygon = polygon.buffer(0)
        if polygon.is_empty or float(polygon.area) < float(minimum_area):
            continue
        inner = polygon.buffer(-inset, join_style=2) if inset > 0.0 else polygon
        if inner.is_empty:
            continue
        if inner.geom_type == 'MultiPolygon':
            inner = max(inner.geoms, key=lambda geometry: geometry.area)
        if inner.geom_type != 'Polygon' or float(inner.area) < float(minimum_area):
            continue
        vertices = [(float(x), float(y)) for x, y in inner.exterior.coords]
        zone_dict[space_name] = {
            'vertices': vertices,
            'height': walls_local[-1]['height'],
            'storey': walls_local[-1]['storey'],
        }
        print(
            f"Space {space_name}: closed wall-axis polygon accepted "
            f"(clear area {inner.area:.2f} m2)"
        )
    return zone_dict
...
def convert_to_dictionary(final_spaces, connection_tolerance=0.15):
    """Order closed space boundaries without inventing missing wall edges.

    The previous fallback appended the nearest endpoint and discarded its
    segment whenever no exact connection was found.  That could turn an open
    set of walls into an apparently closed IfcSpace.  Small endpoint noise is
    still snapped, but a boundary with a real gap is rejected.
    """
    zone_dict = {}
    ...
    for space, walls_local in final_spaces.items():
        ...
        while remaining_walls:
            last_point = np.asarray(ordered_points[-1], dtype=float)
            candidates = []
            for i, wall in enumerate(remaining_walls):
                start = tuple(map(float, wall['start_point']))
                end = tuple(map(float, wall['end_point']))
                candidates.append((
                    float(np.linalg.norm(last_point - np.asarray(start))),
                    i,
                    end,
                ))
                candidates.append((
                    float(np.linalg.norm(last_point - np.asarray(end))),
                    i,
                    start,
                ))

            closest_distance, closest_wall_index, next_point = min(
                candidates, key=lambda item: item[0])
            if closest_distance > float(connection_tolerance):
                print(
                    f"Warning: Open boundary in space {space} "
                    f"(gap {closest_distance:.3f} m). Skipping this space."
                )
                boundary_is_closed = False
                break

            # Snap only the small joint and keep the full connected segment.
            ordered_points.append(next_point)
            remaining_walls.pop(closest_wall_index)

        if not boundary_is_closed:
            continue

        closing_gap = float(np.linalg.norm(
            np.asarray(ordered_points[-1], dtype=float) -
            np.asarray(ordered_points[0], dtype=float)
        ))
        if closing_gap > float(connection_tolerance):
            print(
                f"Warning: Open boundary in space {space} "
                f"(closing gap {closing_gap:.3f} m). Skipping this space."
            )
            continue
...
def identify_zones(walls_local, snapping_distance=0.5, plot_zones=True):
    ...
    return spaces_from_closed_axis_polygons(space_dimensions, updated_walls)
