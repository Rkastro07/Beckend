...
def identify_slabs(points_xyz, points_rgb, bottom_floor_slab_thickness, top_floor_ceiling_thickness,
    ...
    # Agrupa superficies em LAJES por proximidade vertical (gap < merge_dist =
    # mesma laje, faces inferior+superior). Substitui o pareamento por paridade,
    # que assumia exatamente 1 superficie exposta na laje do terreo e 2 nas
    # demais -- nuvens sinteticas (e scans que veem a face de baixo) quebram isso.
    merge_dist = 0.9
    groups = []
    for i, itv in enumerate(h_surf_candidates):
        if groups and itv[0] - h_surf_candidates[groups[-1][-1]][1] < merge_dist:
            groups[-1].append(i)
        else:
            groups.append([i])
    ...
    for gi, g in enumerate(groups):
        pts_g = np.concatenate([horiz_surface_planes[i] for i in g], axis=0)
        z_med = [np.median(horiz_surface_planes[i][:, 2]) for i in g]
        if len(g) >= 2:
            slab_bottom_z_coord = float(min(z_med))
            slab_thickness = float(max(z_med) - min(z_med))
        elif gi == 0:
            # terreo com so a face de cima visivel: espessura nominal pra baixo
            slab_bottom_z_coord = float(z_med[0]) - bottom_floor_slab_thickness
            slab_thickness = bottom_floor_slab_thickness
        elif gi == len(groups) - 1:
            # cobertura com so a face de baixo visivel: espessura nominal pra cima
            slab_bottom_z_coord = float(z_med[0])
            slab_thickness = top_floor_ceiling_thickness
        else:
            # laje intermediaria com 1 face (a de cima, mais escaneada): desce
            slab_bottom_z_coord = float(z_med[0]) - top_floor_ceiling_thickness
            slab_thickness = top_floor_ceiling_thickness
        dil = 1.0 if gi == 0 else 1.5
        print('Creating hull for slab no. %d of %d.' % (gi + 1, len(groups)))
        x_coords, y_coords, polygon = create_hull_from_histogram(pts_g, pc_resolution,
                                                                 grid_coefficient=5, plot_graphics=True,
                                                                 dilation_meters=dil, erosion_meters=dil)
        ...
        print('Slab no. %d: bottom (z-coordinate) = %.3f m, thickness = %0.1f mm'
              % (gi, slab_bottom_z_coord, slab_thickness * 1000))
...
def identify_walls(pointcloud, pointcloud_resolution, minimum_wall_length, minimum_wall_thickness,
    ...
    z_section_boundaries = [float(os.environ.get('WALL_ZLO', 0.85)),
                            float(os.environ.get('WALL_ZHI', 1.2))]  # percentage of the height for the storey sections
