...
def register_tools(app, upload_folder, output_folder, valid_upload):
    ...
    def _planta_parse():
        """DXF -> modelo editavel (JSON). O front desenha/edita esse modelo."""
        try:
            f = request.files.get('file')
            if not f or not valid_upload(f, ('.dxf',)):
                return jsonify({'error': 'Envie um arquivo .dxf'}), 400
            escala = request.form.get('escala')
            escala = float(escala) if escala else None
            esp_default = float(request.form.get('esp_default', 0.15))

            sid = uuid.uuid4().hex[:8]
            dxf_path = upload_folder / f"{sid}_{secure_filename(f.filename)}"
            f.save(str(dxf_path))

            pl = _import_planta()
            with contextlib.redirect_stdout(io.StringIO()):
                modelo = pl.parse_dxf(dxf_path, escala_forcada=escala,
            ...
            traceback.print_exc()
            return jsonify({'error': f'Falha ao interpretar DXF: {e}'}), 500
