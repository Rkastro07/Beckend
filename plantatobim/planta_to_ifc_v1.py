...
=========================
Pipeline:
  1. Le o DXF e coleta segmentos das layers de parede (Wall-*, Parede*)
...
Limitacoes v1 (documentadas):
  - Sem portas/janelas (sem IfcOpeningElement) — paredes ficam cheias
  - So paredes retas (arcos sao ignorados no pareamento)
  - 1 pavimento por DXF
...
def ler_segmentos_parede(dxf_path, escala_forcada=None):
    ...
    for e in msp:
        ...
        if t == "LINE":
            ...
            brutos.append((s.x, s.y, en.x, en.y, layer))
        elif t == "LWPOLYLINE":
            pts = list(e.get_points("xy"))
            rng = list(range(len(pts) - 1))
            if e.closed and len(pts) > 2:
                rng.append(-1)
            for i in rng:
                a, b = pts[i], pts[(i + 1) % len(pts)]
                brutos.append((a[0], a[1], b[0], b[1], layer))
...
def construir_ifc(paredes, laje_poly, altura, esp_laje, pavimento, projeto, out_path,
    ...
    def _caixa(prod_classe, nome, larg, esp, alt, x, y, z, ang):
    ...
    def _esquadria_multi(prod_classe, nome, solids, x, y, z, ang):
    ...
    def _porta_detalhada(nome, larg, esp, alt, x, y, z, ang):
        ...
        return _esquadria_multi("IfcDoor", nome, solids, x, y, z, ang)

    def _janela_detalhada(nome, larg, esp, alt, x, y, z, ang):
        ...
        return _esquadria_multi("IfcWindow", nome, solids, x, y, z, ang)
    ...
    for i, p in enumerate(paredes, 1):
        ...
        wall = _caixa("IfcWall", f"Parede-{i:03d}",
                      p["comprimento"], p["espessura"], altura,
                      float(A[0]), float(A[1]), 0.0, ang)
    ...
    for ab in (aberturas or []):
        ...
        larg = ab["largura"]
        if ab["tipo"] == "door":
            z0, alt = 0.0, porta_altura
            n_p += 1
            classe, nome = "IfcDoor", f"Porta-{n_p:03d}"
        else:
            z0, alt = janela_peitoril, janela_altura
            n_j += 1
            classe, nome = "IfcWindow", f"Janela-{n_j:03d}"
        ...
        if (larg - dentro) > 0.05 and (z0 + alt) < altura - 1e-3:
            verga = _caixa("IfcWall", f"Verga-{nome}",
                           larg, p["espessura"], altura - (z0 + alt),
...
def modelo_para_dict(modelo):
    ...
    return {"escala": modelo.get("escala"), "single_line": modelo.get("single_line"),
            "diagnostico": {"sobras": modelo.get("n_sobras", 0),
                            "cantos_costurados": modelo.get("n_cantos", 0),
                            "blocos_esquadria": modelo.get("n_blocos_esq", 0)},
            "bbox": bbox, "paredes": paredes, "aberturas": aberturas, "laje": laje}
...
def dict_para_modelo(d):
    ...
    for w in d.get("paredes", []):
        ...
        paredes.append({"eixo": (A, B), "espessura": float(w["espessura"]),
    ...
    for o in d.get("aberturas", []):
        ...
        aberturas.append({"parede_idx": id2idx[o["parede_id"]], "tipo": o["tipo"],
